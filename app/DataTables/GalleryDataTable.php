...
class GalleryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function (Gallery $Gallery) {
                ...
                $status .= '<input class="form-check-input change-status-input" type="checkbox" id="flexSwitchCheckChecked" data-carousel-id="' . $Gallery->id . '" ' . ($Gallery->status == 1 ? 'checked' : '') . '>';
                $status .= '</div>';
            ...
            ->addColumn('created_at', function (Gallery $Gallery) {
                $dates = 'Created: ' . $Gallery->created_at->diffForHumans() . '<br><hr/>';
                $dates .= 'Updated: ' . $Gallery->updated_at->diffForHumans();
                return new HtmlString($dates);
            })
            ->addColumn('action', function (Gallery $Gallery) {
                return '<a class="dropdown-item" href="' . route("admin.home-page-carousel.edit", $Gallery) . '"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                        <a class="dropdown-item delete-plan" href="' . route("admin.home-page-carousel.destroy", $Gallery) . '" onclick="return confirm(\'Are you sure?\')" data-id="' . $Gallery->id . '"><i class="bx bx-trash me-1"></i> Delete</a>';
            })
            ->addColumn('image', function (Gallery $Gallery) {
                return '<img class="w-25 img-thumbnail imgRenderContainer" src="' . public_asset($Gallery->image) . '" alt="' . $Gallery->title . '">';
            })
     ...
     */
    public function query(Gallery $Gallery): QueryBuilder
    {
        return $Gallery->newQuery();
    }
                ...
                Button::make()
                    ...
                    ->action(
                        'function(e, dt, node, config) {
                            let url = "' . route('admin.pages.gallery.create') . '";
            ...
            ->parameters([
                ...
                'initComplete' => 'function () {
                    $(".change-status-input").change(function() {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "' . route('admin.pages.gallery.change.status') . '",
                            data: requestData,
                            dataType: "json",
