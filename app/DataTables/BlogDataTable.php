...
class BlogDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function (Blog $blog) {
                ...
                $status .= '<input class="form-check-input change-status-input" type="checkbox" id="flexSwitchCheckChecked" data-blog-id="' . $blog->id . '" ' . ($blog->status == 1 ? 'checked' : '') . '>';
                $status .= '</div>';
            ...
            ->addColumn('created_at', function (Blog $blog) {
                $dates = 'Created: ' . $blog->created_at->diffForHumans() . '<br><hr/>';
                $dates .= 'Updated: ' . $blog->updated_at->diffForHumans();
                return new HtmlString($dates);
            })
            ->addColumn('action', function (Blog $blog) {
                return '<a class="dropdown-item" href="' . route("admin.blog.edit", $blog) . '"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                        <a class="dropdown-item delete-plan" href="' . route("admin.blog.delete", $blog) . '" onclick="return confirm(\'Are you sure?\')" data-id="' . $blog->id . '"><i class="bx bx-trash me-1"></i> Delete</a>';
            })
     ...
     */
    public function query(Blog $blog): QueryBuilder
    {
                ...
                Button::make()
                    ...
                    ->action(
                        'function(e, dt, node, config) {
                            let url = "' . route('admin.blog.create') . '";
